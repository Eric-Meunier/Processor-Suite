...
#         tag_string = ""
#
#         tag_string += tag('FMT', tags['Format'])
#         tag_string += tag('UNI', tags['Unit'])
#         tag_string += tag('OPR', tags['Operator'])
#         tag_string += tag('XYP', ' '.join([str(n) for n in tags['XYProbe']]))
#         tag_string += tag('CUR', tags['PeakLoopCurrent'])
#         tag_string += tag('TXS', ' '.join([str(n) for n in tags['LoopSize']]))
#
#         tag_string += '~ Transmitter Loop Co-ordinates:\n'
#         cnt = 0
#         for loop_coord in tags['LoopCoords']:
#             loop_coord = [str(c) for c in loop_coord]
#             tag_string += tag('L'+ ('0' + str(cnt) if cnt < 10 else str(cnt)), ' '.join(loop_coord))
#             cnt += 1
#
#         tag_string += '~ Hole/Profile Co-ordinates:\n'
#         cnt = 0
#         for hole_coord in tags['HoleCoords']:
#             hole_coord = [str(c) for c in hole_coord]
#             tag_string += tag('P' + ('0' + str(cnt) if cnt < 10 else str(cnt)), ' '.join(hole_coord))
#             cnt += 1
#
#         return tag_string
#
...
#         :return: A string in PEM file foh forgot to say, one reason i picked ormat containing the data found inside of pem_file
#         """
#
